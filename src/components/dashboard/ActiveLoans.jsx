...
const ActiveLoans = ({ walletAddress }) => {
  const { updateTrustScore } = useTrustScoreUpdater();
  ...
  const handleRepay = async (loanId) => {
    try {
      toast.loading(`Processing repayment for Loan ${loanId}...`, { id: loanId });
      await updateTrustScore(10); // +10 score on repay
      toast.success(`Loan ${loanId} repaid successfully! TrustScore updated.`, { id: loanId });
    } catch (err) {
      toast.error(`Error repaying Loan ${loanId}`, { id: loanId });
    }
  };

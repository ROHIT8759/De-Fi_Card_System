...
// // const LoanEligibilityMeter = ({ score, factors }) => {
...
// const LoanEligibilityMeter = ({ walletAddress, score: propScore, factors: propFactors }) => {
...
//         { name: 'Income Stability', score: Math.min(100, score + 5), weight: 25 },
//         { name: 'Debt-to-Income', score: Math.max(0, score - 10), weight: 20 },
//         { name: 'Collateral Value', score: Math.min(100, score + 10), weight: 15 },
//         { name: 'On-chain Activity', score: Math.max(0, score - 5), weight: 10 }
//       ];
//     }

//     // Calculate factors based on actual on-chain metrics
//     const loanHistoryScore = detailed.loan_count > 0
//       ? Math.min(100, (parseInt(detailed.total_repaid) / Math.max(1, parseInt(detailed.total_borrowed))) * 100)
//       : score;

//     const stakingScore = parseInt(detailed.staked_amount) > 0
//       ? Math.min(100, Math.log10(parseInt(detailed.staked_amount) / 1e8 + 1) * 30 + 40)
//       : Math.max(0, score - 20);

//     const defaultScore = parseInt(detailed.defaults) === 0 ? 100 : Math.max(0, 100 - (parseInt(detailed.defaults) * 25));

//     const walletAgeScore = Math.min(100, Math.log10(Math.max(1, parseInt(detailed.wallet_age) / (24 * 60 * 60))) * 20 + 20);

//     const earlyRepaymentScore = parseInt(detailed.early_repayments) > 0
...
//       { name: 'Early Repayments', score: Math.round(earlyRepaymentScore), weight: 10 }
//     ];
...
//             <div className="text-cyan-400 font-bold">{maxLoanAmount.toFixed(2)} APT</div>
...
//             <li className="text-yellow-400">Create a trust score to see real metrics</li>
//           )}
//           {score < 60 && (
//             <>
//               <li>Improve credit history with on-time payments</li>
